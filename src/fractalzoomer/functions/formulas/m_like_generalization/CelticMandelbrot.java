...
public class CelticMandelbrot extends Julia {
    ...
    public void function(Complex[] complex) {
        ...
        complex[0].square_mutable().absre_mutable().plus_mutable(complex[1]);
    ...
    public Complex perturbationFunction(Complex DeltaSubN, Complex DeltaSub0, int RefIteration) {
        ...
        double xrn = Complex.DiffAbs(Xr * Xr - Xi * Xi, (2 * Xr + xr) * xr - (2 * Xi + xi) * xi);
        double xin = 2 * (Xr * xi + xr * (Xi + xi));
    ...
    public MantExpComplex perturbationFunction(MantExpComplex DeltaSubN, ReferenceDeepData data, int RefIteration) {
        ...
        MantExp xrn = MantExpComplex.DiffAbs(Xr.square().subtract_mutable(Xi.square()), Xr.multiply2().add_mutable(xr).multiply_mutable(xr).subtract_mutable(Xi.multiply2().add_mutable(xi).multiply_mutable(xi)));
        MantExp xin = Xr.multiply(xi).add_mutable(Xi.add(xi).multiply_mutable(xr)).multiply2_mutable();
    ...
    protected GenericComplex referenceFunction(GenericComplex z, GenericComplex c, NormComponents normData, GenericComplex[] initialPrecal, GenericComplex[] precalc) {
        ...
        if(normData != null) {
            z = z.squareFast_mutable(normData).absre_mutable().plus_mutable(c);
        }
        else {
    ...
    public Complex function(Complex z, Complex c) {
        return z.square_mutable().absre_mutable().plus_mutable(c);
    }
