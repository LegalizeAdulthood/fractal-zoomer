...
public class PerpendicularBurningShip extends Julia {
    ...
    public void function(Complex[] complex) {
        ...
        complex[0].absNegateIm_mutable().square_mutable().plus_mutable(complex[1]);
    ...
    public Complex perturbationFunction(Complex DeltaSubN, Complex DeltaSub0, int RefIteration) {
        ...
        double xrn = 2 * (Xr * xr - xi * Xi) + xr2 - xi2;
        double xin = -2 * (Complex.DiffAbs(Xi, xi) * Xr + xr * Math.abs((Xi + xi)));
    ...
    public MantExpComplex perturbationFunction(MantExpComplex DeltaSubN, ReferenceDeepData data, int RefIteration) {
        ...
        MantExp xrn = Xr.multiply(xr).subtract_mutable(xi.multiply(Xi)).multiply2_mutable().add_mutable(xr2).subtract_mutable(xi2);
        MantExp xin = MantExpComplex.DiffAbs(Xi, xi).multiply_mutable(Xr).add_mutable(xr.multiply(Xi.add(xi).abs_mutable())).multiply2_mutable().negate_mutable();
    ...
    public Complex function(Complex z, Complex c) {
        return z.absNegateIm_mutable().square_mutable().plus_mutable(c);
    }
